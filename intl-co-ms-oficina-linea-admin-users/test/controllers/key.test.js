...
describe("Login controller tests", () => {
    ...
    test("Must to return array length > 1 and first item contains businessName on valid data requestGetInfoKeys", async () => {
        const data = await requestGetInfoKeys(KeyEvent);
        expect(data.length).toBeGreaterThan(0);
        expect(data[0].businessName).toEqual("BUSINESS NAME");
    });

    test("Must to return 422 on invalid data requestGetInfoKeys", async () => {
        const { status } = await requestGetInfoKeys({});
